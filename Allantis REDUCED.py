# -*- coding: utf-8 -*-
"""
ALLANTIS REDUCED SPX — SISTEMA FWD (Forward Testing) con Mediana Intraday — LOCAL DATA
========================================================================================
VERSIÓN: Allantis REDUCED — Butterfly PURA de Puts (sin Calendar Call)

ESTRUCTURA ALLANTIS REDUCED:
=============================
PATAS y VENCIMIENTOS:
- Puts: TODO en DTE1 (aprox 160 DTE) → Butterfly de puts: +4P@UW : -8P@Body : +4P@LW
  * Upper Wing (UW): Long 4P aprox Δ40 (ala superior de la Butterfly)
  * Body: Short 8P aprox Δ30 (cuerpo central de la Butterfly)
  * Lower Wing (LW): Long 4P aprox Δ20 (ala inferior de la Butterfly, deep OTM)

TOTAL: 3 grupos de patas con 1 expiración (DTE1 para todas las puts)

GRUPOS DE STRIKES (basados en rangos delta):
1. UW_DELTA: Upper Wing puts (Δ40 puts) - Rango delta para long puts superiores
2. BODY_DELTA: Body puts centrales (Δ30 puts) - Rango delta para shorts centrales
3. LW_DELTA: Lower Wing puts (Δ20 puts) - Rango delta para long puts deep OTM

EXPIRACIONES:
- RANGE_A: DTE1 (aprox 160 DTE) para Butterfly puts completa

CAMBIOS vs ALLANTIS COMPLETO:
1) **Estructura simplificada a 3-leg Butterfly**:
   - Butterfly de puts: +4P:-8P:+4P (3 strikes, DTE1)
   - ELIMINADO: Calendar call completo (-2C@DTE1:+2C@DTE2)
   - Total: 3 grupos de patas, 16 contratos
   - Función `butterfly_value_from_df`: revaloriza estructura de 3 patas
   - Cálculo de net_credit simplificado (3 grupos, 1 expiración)

2) **Puntos geométricos simplificados**:
   - LEL: Asíntota inferior (S < k_lw)
   - BE1: Primer breakeven
   - PICO: Máximo de la Butterfly (en k_body)
   - BE2: Segundo breakeven
   - UEL: Asíntota superior (S > k_uw) - SIMPLIFICADA a -net_debit
   - ELIMINADO: VALLE, BreakEvenCCAL1, BreakEvenCCAL2, PICOCCAL, MASTER_DISTANCE

3) **BQI (Butterfly Quality Index)**:
   - Reemplaza AQI, basado en 4 componentes: LEL, UEL, PICO, Theta
   - Más simple que AQI (sin métricas de calendar call)

4) **Sistema FWD con mediana intraday**:
   - Evalúa estructura en múltiples timestamps fijos US
   - Agrega por MEDIANA → columnas: PnL_fwd_pts_*_mediana y PnL_fwd_pct_*_mediana
   - Optimización: lee archivo FWD UNA SOLA VEZ por ventana

5) **Paralelización con ProcessPoolExecutor**:
   - Worker function para cálculo paralelo
...
RANKING_MODE = "BQI"             # Métrica de ranking para ordenar estructuras Butterfly
                                          # "theta_total": Theta total
                                          # "BQI": Butterfly Quality Index (balance entre LEL, UEL, PICO, theta)
...
FILTER_BQI_THRESHOLD = 2.0            # Umbral para gráficos de BQI (Butterfly Quality Index)
                                       # BQI balancea: LEL, UEL, PICO, theta
...
# === DTE (Days To Expiration) - Rango para expiración única ===
# Se procesan TODAS las expiraciones dentro del rango (sin límites de cantidad)
RANGE_A = (120, 180)       # ! Rango DTE para DTE1 (aprox 160 DTE)
                           # Usado para: Butterfly puts completa (+4P:-8P:+4P)
                           # Ejemplos alternativos:
                           # (140, 180): Ventana más amplia alrededor de 160 DTE
                           # (155, 165): Ventana muy estrecha para mayor precisión

# NOTA: RANGE_B eliminado - ALLANTIS REDUCED usa solo DTE1

# === ESTRUCTURA BUTTERFLY REDUCED: 3 GRUPOS DE STRIKES (SOLO PUTS) ===
# Estructura: +4P@UW(DTE1) : -8P@Body(DTE1) : +4P@LW(DTE1)
#
# PUTS (todos en DTE1):
# 1. UW (Upper Wing): Long 4P aprox Δ40 (ala superior Butterfly)
# 2. Body: Short 8P aprox Δ30 (cuerpo central Butterfly)
# 3. LW (Lower Wing): Long 4P aprox Δ20 (ala inferior Butterfly, deep OTM)

# === UW PUTS (Upper Wing - Long 4P) - BASADO EN DELTA ===
# Ala superior de la Butterfly, establece el límite superior
# Delta de put: valor entre -1 y 0 (en magnitud: 0% a 100%)
UW_PUT_DELTA_MIN = 0.35      # ! Delta mínimo para UW puts (Δ40 target)
UW_PUT_DELTA_MAX = 0.45      # ! Delta máximo para UW puts
                             # Rango típico: [0.35, 0.45] para capturar Δ40
                             # Estos puts están más ITM que los body

# === BODY PUTS (Body - Short 8P) - BASADO EN DELTA ===
# Cuerpo central de la Butterfly, corazón de la estructura
BODY_PUT_DELTA_MIN = 0.25    # ! Delta mínimo para body puts (Δ30 target)
BODY_PUT_DELTA_MAX = 0.35    # ! Delta máximo para body puts
                             # Rango típico: [0.25, 0.35] para capturar Δ30
                             # Strike constraint: UW > Body

# === LW PUTS (Lower Wing - Long 4P) - BASADO EN DELTA ===
# Ala inferior de la Butterfly, deep OTM, establece el límite inferior
LW_PUT_DELTA_MIN = 0.15      # ! Delta mínimo para LW puts (Δ20 target)
LW_PUT_DELTA_MAX = 0.25      # ! Delta máximo para LW puts
                             # Rango típico: [0.15, 0.25] para capturar Δ20
                             # Strike constraint: UW > Body > LW

# === Comentarios de referencia ===
# Estructura Butterfly Reduced necesita 3 strikes:
# - K_UW (puts Upper Wing, Δ40)
# - K_Body (puts Body, Δ30)
# - K_LW (puts Lower Wing, Δ20)
# Constraint: K_UW > K_Body > K_LW (todos puts, strikes decrecientes hacia OTM)


# === PREFILTRO NET CREDIT (Crédito inicial de la estructura Butterfly) ===
# Butterfly Reduced: +4P -8P +4P = 16 contratos totales
# Net credit = +4*P_UW - 8*P_Body + 4*P_LW (en puntos SPX)
...
# DELTA: Butterfly Reduced tiene 16 contratos (solo BWB puts)
# BWB puts: delta negativo (protección bajista)
# Estructura debería ser aproximadamente delta-neutral
DELTA_MIN, DELTA_MAX = -0.05, 0.05    # Rango delta total para 16 contratos
                                       # BWB: ~4*(-0.40) - 8*(-0.30) + 4*(-0.20) = -1.6 + 2.4 - 0.8 = 0
                                       # Total esperado: cerca de 0, permitir ±0.05 para variabilidad

# THETA: 16 contratos = theta positivo (decay favorable en BWB)
# Butterfly pura de puts tiene theta positivo por los shorts
THETA_MIN, THETA_MAX = 20, 50000.0  # Theta diario en USD
                                         # Butterfly típico: 40-150 USD/día positivo
...
# === FILTRO BQI (Butterfly Quality Index) ===
# Métrica de calidad específica para Butterfly pura de Puts
# Balancea LEL, UEL, PICO, theta (simplificado vs AQI de Allantis completo)
FILTER_BQI_ENABLED = True             # True: aplica filtro por BQI | False: no filtra
BQI_MIN = 0                         # BQI mínimo aceptable (valores típicos: 1-5)
BQI_MAX = 100000                      # Sin límite superior
CALCULATE_BQI = True                  # Control para cálculo de BQI
...
PREFERRED_COLUMN_ORDER = [
    ...
    "dia",
    "net_credit",
    "k_uw",                   # Strikes de Butterfly (3 patas)
    "k_body",
    "k_lw",
    # IVs de las 3 patas
    "iv_uw",
...
def bs_theta_put_excel(S,K,T,r,sigma):
    """Black-Scholes theta para put (anualizado)."""
    if T<=0 or sigma<=0:
        return 0.0
    sqrtT = math.sqrt(T)
    d1 = (math.log(S/K) + (r + 0.5*sigma*sigma)*T) / (sigma*sqrtT)
    ...
    pdf = math.exp(-0.5*d1*d1)/math.sqrt(2*math.pi)
    term1 = -(S * pdf * sigma) / (2*sqrtT)
    term2 = r * K * math.exp(-r*T) * normal_cdf(-d2)  # Nota: +r para put (vs -r para call)
    return term1 + term2
...
# ================== ALLANTIS: FUNCIONES DE GENERACIÓN DE CANDIDATOS ==================

def bs_put_price(S, K, T, r, sigma, q=0.0):
    """Black-Scholes precio de put."""
    if T <= 0 or sigma <= 0:
        return max(K - S, 0.0)
    d1 = (math.log(S/K) + (r - q + 0.5*sigma*sigma)*T) / (sigma*math.sqrt(T))
    d2 = d1 - sigma*math.sqrt(T)
...
def bs_delta_put(S, K, T, r, sigma, q=0.0):
    """Black-Scholes delta de put (negativo)."""
    if T <= 0 or sigma <= 0:
        return -1.0 if S < K else 0.0
    d1 = (math.log(S/K) + (r - q + 0.5*sigma*sigma)*T) / (sigma*math.sqrt(T))
    return -math.exp(-q*T)*normal_cdf(-d1)

def implied_vol_put_from_price(S, K, T, r, q, target_price,
                                lo=1e-6, hi=5.0, tol=1e-7, max_iter=100):
    """IV inversa para put: asegura precio ≥ intrínseco, bisección con fallback Newton."""
    ...
    def f(sig):
        return bs_put_price(S, K, T, r, sig, q) - target
    ...
    if f_lo * f_hi > 0:
        ...
        sigma = 0.2
        for _ in range(max_iter):
            price = bs_put_price(S, K, T, r, sigma, q)
            vega = bs_vega_call(S, K, T, r, sigma, q)  # Vega es igual para call y put
            if vega <= 1e-12:
                break
            diff = price - target
            sigma = max(lo, min(hi, sigma - diff / max(vega, 1e-12)))
...
def gen_uw_put_candidates_by_delta(puts_idx, available_strikes, spot, dte, r,
    ...
    Args:
        ...
        dte: Days to expiration
        r: Risk-free rate
        delta_min: Delta mínimo aceptable en magnitud (ej: 0.35 = 35%)
        delta_max: Delta máximo aceptable en magnitud (ej: 0.45 = 45%)
        q: Dividend yield (default 0.0)
    ...
    for K in available_strikes:
        ...
        # Calcular IV
        iv = implied_vol_put_from_price(spot, K, T, r, q, mid)
        if iv is None or (isinstance(iv, float) and (math.isnan(iv) or iv <= 0)):
            continue

        # Calcular delta (en magnitud, put delta es negativo)
        delta_put = bs_delta_put(spot, K, T, r, iv, q)
        delta_mag = abs(delta_put)  # Magnitud del delta

        # Filtrar por rango de delta en magnitud
        if delta_min <= delta_mag <= delta_max:
            candidates.append(K)
...
def gen_body_put_candidates_by_delta(puts_idx, available_strikes, spot, dte, r,
    ...
    Args:
        ...
        dte: Days to expiration
        r: Risk-free rate
        delta_min: Delta mínimo aceptable en magnitud (ej: 0.25 = 25%)
        delta_max: Delta máximo aceptable en magnitud (ej: 0.35 = 35%)
        q: Dividend yield (default 0.0)
...
def gen_lw_put_candidates_by_delta(puts_idx, available_strikes, spot, dte, r,
    ...
    Args:
        ...
        dte: Days to expiration
        r: Risk-free rate
        delta_min: Delta mínimo aceptable en magnitud (ej: 0.15 = 15%)
        delta_max: Delta máximo aceptable en magnitud (ej: 0.25 = 25%)
        q: Dividend yield (default 0.0)
...
def gen_short_call_candidates_by_delta(calls_idx, available_strikes, spot, dte, r,
                                        delta_min=0.25, delta_max=0.35, q=0.0):
    """
    Genera candidatos de Short calls (short 2C @ DTE1) filtrando por rango de delta.

    Args:
        calls_idx: DataFrame indexado por strike con datos de calls
        available_strikes: Lista de strikes disponibles
        spot: Precio subyacente (SPX)
        dte: Days to expiration
        r: Risk-free rate
...
# ================== MÉTRICAS BUTTERFLY (3 LEGS: BWB PUTS PURA) ==================
def compute_butterfly_metrics(spot, r_base, exp1, dte1, k_uw, k_body, k_lw, precache):
    """
    Calcula métricas para estructura Butterfly de 3 patas:
    - Butterfly de puts @ DTE1: +4P@UW : -8P@Body : +4P@LW

    Args:
        spot: precio actual SPX
        r_base: tasa libre de riesgo
        exp1: expiration DTE1 (RANGE_A, ~160 días)
        dte1: días hasta exp1
        k_uw: strike Upper Wing puts (long 4P)
        k_body: strike Body puts (short 8P)
        k_lw: strike Lower Wing puts (long 4P)
    ...
    """
    # Obtener datos de puts para DTE1
    ...
    def greeks_price_put(idx: pd.DataFrame, exp, K, T, r, q):
        """Obtiene greeks y precio para una put."""
        key = ("put", exp, float(K), round(T, 6))
        if key in GREEKS_CACHE:
            return GREEKS_CACHE[key]
        ...
        # Usar greeks del snapshot si están disponibles
        delta_snapshot = float(row.get("delta_BS", np.nan))
        theta_snapshot = float(row.get("theta_BS", np.nan))
        iv_snapshot = float(row.get("IV_BS", np.nan))
        ...
        # Calcular greeks manualmente
        delta = bs_delta_put(spot, K, T, r, iv, q)
        theta_annual_long = bs_theta_put_excel(spot, K, T, r, iv)
        theta_daily = theta_annual_long * THETA_TO_DAILY
        GREEKS_CACHE[key] = (delta, theta_daily, mid, iv, bid, ask, last)
        return GREEKS_CACHE[key]

    # Obtener greeks y precios de las 3 patas de puts @ DTE1
    d_uw, t_uw, p_uw, iv_uw, bid_uw, ask_uw, last_uw = greeks_price_put(puts1_idx, exp1, k_uw, T1, r1, q1)  # +4P UW
    d_body, t_body, p_body, iv_body, bid_body, ask_body, last_body = greeks_price_put(puts1_idx, exp1, k_body, T1, r1, q1)  # -8P Body
    d_lw, t_lw, p_lw, iv_lw, bid_lw, ask_lw, last_lw = greeks_price_put(puts1_idx, exp1, k_lw, T1, r1, q1)  # +4P LW

    # Helper para convertir NaN a 0
    dz = lambda x: 0.0 if (x is None or (isinstance(x, float) and math.isnan(x))) else x

    # Calcular delta y theta totales
    # Estructura: +4P@UW -8P@Body +4P@LW
    delta_total = (
        +4 * dz(d_uw)
        - 8 * dz(d_body)
        + 4 * dz(d_lw)
    ...
    # Calcular net_credit (puntos SPX)
    # Estructura: +4P@UW -8P@Body +4P@LW
    p_uw_z = dz(p_uw)
    p_body_z = dz(p_body)
    p_lw_z = dz(p_lw)

    net_credit = (+4 * p_uw_z) + (-8 * p_body_z) + (+4 * p_lw_z)

    # Formatear IVs
    iv_uw_fmt = None if (iv_uw is None or (isinstance(iv_uw, float) and math.isnan(iv_uw))) else round(float(iv_uw), 6)
    iv_body_fmt = None if (iv_body is None or (isinstance(iv_body, float) and math.isnan(iv_body))) else round(float(iv_body), 6)
    iv_lw_fmt = None if (iv_lw is None or (isinstance(iv_lw, float) and math.isnan(iv_lw))) else round(float(iv_lw), 6)

    # ================== CÁLCULO DE PUNTOS GEOMÉTRICOS BUTTERFLY @ VENCIMIENTO DTE1 ==================
    # Estructura: Butterfly pura de Puts
    # Puntos: LEL, PuntoLEL, BE1 (BreakEven1), PICO, BE2 (BreakEven2), UEL
    ...
    # Cuando S < k_lw, todas las puts están ITM
    # PnL = 4·(k_uw - S) - 8·(k_body - S) + 4·(k_lw - S) - net_debit
    #     = 4k_uw - 8k_body + 4k_lw - net_debit (constante)
    LEL_pts = 4*float(k_uw) - 8*float(k_body) + 4*float(k_lw) - net_debit
    LEL_USD = LEL_pts * 100.0  # Convertir a USD

    # --- 2. PuntoLEL - Punto de inflexión donde comienza el ascenso ---
    # Ubicado en S = k_lw
    PuntoLEL_strike = float(k_lw)
    PuntoLEL_PnL_pts = LEL_pts  # Mismo valor que LEL
    PuntoLEL_PnL_USD = LEL_USD

    # --- 3. BE1 (BreakEven1) - Primer breakeven de la Butterfly ---
    # En región k_lw < S < k_body: PnL = (4k_uw - 8k_body - net_debit) + 4S
    # Resolver: (4k_uw - 8k_body - net_debit) + 4S = 0
    # S = (8k_body - 4k_uw + net_debit) / 4
    BE1_strike = None
    numerator_be1 = 8*float(k_body) - 4*float(k_uw) + net_debit
    if numerator_be1 >= 0:  # Solo válido si S >= 0
        be1_candidate = numerator_be1 / 4.0
        # Verificar que está en rango válido: k_lw < S < k_body
        if float(k_lw) < be1_candidate < float(k_body):
            BE1_strike = be1_candidate

    # --- 4. PICO - Máximo de la Butterfly ---
    # Ubicado en S = k_body
    PICO_strike = float(k_body)
    # PnL = 4·(k_uw - k_body) - net_debit
    PICO_PnL_pts = 4*(float(k_uw) - float(k_body)) - net_debit
    PICO_PnL_USD = PICO_PnL_pts * 100.0

    # --- 5. BE2 (BreakEven2) - Segundo breakeven de la Butterfly ---
    # En región k_body < S < k_uw: PnL = 4k_uw - 4S - net_debit
    # Resolver: 4k_uw - 4S - net_debit = 0
    # S = k_uw - net_debit/4
    BE2_strike = None
    be2_candidate = float(k_uw) - net_debit / 4.0
    # Verificar que está en rango válido: k_body < S < k_uw
...
def _process_one_fwd_allantis(args):
    """
    Worker function para calcular FWD de Allantis en paralelo.
    Evalúa la estructura de 5 patas en 14 timestamps fijos de mercado US y calcula mediana de PnL.
    ...
    Returns:
        dict: resultados FWD para todas las ventanas de este Allantis (incluye medianas)
    """
    (i, row_dict, files_sorted_list, forward_fracs, frac_suffixes,
     tz_us_str, tz_es_str, nearest_minute_tol, intraday_timestamps) = args
    ...
    # Extraer root values del Allantis base
    base_root_exp1_norm = normalize_root_value(row_dict.get("root_exp1"))
    base_root_exp2_norm = normalize_root_value(row_dict.get("root_exp2"))
    base_credit = float(row_dict["net_credit"])
    ...
    for fr, suf in zip(forward_fracs, frac_suffixes):
        ...
        try:
            ...
            # Leer archivo UNA SOLA VEZ para los 14 timestamps
            fwd_df = pd.read_csv(fwd_path)
            if fwd_df.empty:
                continue
            fwd_df = td_rescale_strike_only(fwd_df)
            ...
            # Variables para el primer timestamp válido
            first_valid_timestamp = None
            first_net_credit = None
            first_pnl_pts = None
            first_pnl_pct = None
            ...
            for ts_hhmm in intraday_timestamps:
                ...
                net_credit_val = float(av_details["net_credit"])
                ...
                # Guardar datos del primer timestamp válido
                if first_valid_timestamp is None:
                    first_valid_timestamp = ts_hhmm
                    first_net_credit = net_credit_val
                    first_pnl_pts = pnl_pts
                    ...
                    # Metadata de las 5 patas
                    for leg_label, leg_info in (("ul", av_details.get("leg_ul")),
                                                ("shorts", av_details.get("leg_shorts")),
                                                ("ll", av_details.get("leg_ll")),
                                                ("c_short", av_details.get("leg_c_short")),
                                                ("c_long", av_details.get("leg_c_long"))):
                        if not leg_info:
                            continue
                        bid_v, ask_v, mid_v = leg_info.get("bid"), leg_info.get("ask"), leg_info.get("mid")
                        root_v = normalize_root_value(leg_info.get("root"))
                        ...
                        # Validación de mid
                        if leg_label == "ul":
                            expected_mid = row_dict.get("price_mid_ul")
                        elif leg_label == "shorts":
                            expected_mid = row_dict.get("price_mid_shorts")
                        elif leg_label == "ll":
                            expected_mid = row_dict.get("price_mid_ll")
                        elif leg_label == "c_short":
                            expected_mid = row_dict.get("price_mid_c_short")
                        else:  # c_long
                            expected_mid = row_dict.get("price_mid_c_long")
                        mid_status = compare_mid_values(mid_v, expected_mid)

                        # Validación de root
                        if leg_label in ("ul", "shorts", "ll", "c_short"):
                            base_root_expected = base_root_exp1_norm
                        else:  # c_long
                            base_root_expected = base_root_exp2_norm
                        root_status = "CORRECTO" if (not base_root_expected or root_v==base_root_expected) else "INCORRECTO"
            ...
            if first_valid_timestamp is not None:
                ...
                # Metadata de las 5 patas
                for leg_label in ("ul", "shorts", "ll", "c_short", "c_long"):
                    if leg_label in first_leg_data:
                        ld = first_leg_data[leg_label]
                        result['fwd_data'][f"fwd_file_{leg_label}_{suf}"] = ld['file']
                        result['fwd_data'][f"fwd_row_{leg_label}_{suf}"] = ld['row']
            ...
            for t_idx, target_hhmm_us in enumerate(TARGET_HHMMS_US if not IGNORE_TARGET_MINUTE else ["ALL"], start=1):
                ...
                if INCREMENTAL_MODE:
                    ...
                    for e1 in exp_A:
                        ...
                        ul_list = gen_uw_put_candidates_by_delta(puts1_idx, strikes_puts1, spot, dte_map[e1], r_base, UL_PUT_DELTA_MIN, UL_PUT_DELTA_MAX)
                        if not ul_list:
                            continue
                    ...
                    for e1_idx, e1 in enumerate(exp_A, start=1):
                        ...
                        ul_list = gen_uw_put_candidates_by_delta(puts1_idx, strikes_puts1, spot, dte_map[e1], r_base, UL_PUT_DELTA_MIN, UL_PUT_DELTA_MAX)
                        shorts_list = gen_body_put_candidates_by_delta(puts1_idx, strikes_puts1, spot, dte_map[e1], r_base, SHORT_PUT_DELTA_MIN, SHORT_PUT_DELTA_MAX)
                        ll_list = gen_lw_put_candidates_by_delta(puts1_idx, strikes_puts1, spot, dte_map[e1], r_base, LL_PUT_DELTA_MIN, LL_PUT_DELTA_MAX)
