# -*- coding: utf-8 -*-
"""
BATMAN SPX V23 4-LEGS — SISTEMA FWD con estructura segregada de DTE2 (+1C +1C)
===========================================================================
...
import os, math, calendar, re, random, urllib.request, csv, tempfile
...
RANGE_A = (60, 9999)       # ! Rango DTE para expiration FRONT (patas cortas k1 y k3)
...
RANGE_B = (90, 999)       # ! Rango DTE para expiration BACK (pata larga k2)
...
BASE_K1_DELTA_MIN = 0.30     # ! Delta mínimo para K1 (50%)
BASE_K1_DELTA_MAX = 0.70     # ! Delta máximo para K1 (60%)
...
K2_DELTA_MIN = 0.20          # ! Delta mínimo para K2 (primera +1C de DTE2)
K2_DELTA_MAX = 0.40          # ! Delta máximo para K2 (primera +1C de DTE2)
...
# === K2b CANDIDATOS (Strike segunda pata larga back) - BASADO EN DELTA ===
# Los strikes candidatos para K2b se seleccionan según su delta (segunda +1C de DTE2)
# IMPORTANTE: K2b debe ser > K2 > K1 para estructura ordenada
K2b_DELTA_MIN = 0.15         # ! Delta mínimo para K2b (segunda +1C de DTE2)
K2b_DELTA_MAX = 0.30         # ! Delta máximo para K2b (segunda +1C de DTE2)
                             # EJEMPLO: [0.15, 0.30] permite strikes con deltas entre 15% y 30%
                             # NOTA: Típicamente K2b tendrá delta menor que K2 (más OTM)
...
# ============================================================================
# CONFIGURACIÓN DE MODO INCREMENTAL (similar a ALLANTIS)
# ============================================================================
INCREMENTAL_MODE = True      # True: Modo incremental (procesa 1 a 1, bajo uso memoria)
...
PREFILTER_CREDIT_MAX = -5    # ! Crédito máximo aceptable: -2 pts = recibir máximo $200 por contrato
...
def gen_k2b_candidates_by_delta(calls_idx, available_strikes, spot, dte, r,
                                 delta_min=0.15, delta_max=0.30, q=0.0):
    """
    Genera candidatos de K2b (segunda pata larga back) filtrando strikes por rango de delta.
    ...
    Args:
        ...
        dte: Days to expiration
        r: Risk-free rate
        delta_min: Delta mínimo aceptable (ej: 0.15 = 15%)
        delta_max: Delta máximo aceptable (ej: 0.30 = 30%)
        q: Dividend yield (default 0.0)
...
# ================== URL ==================
def make_url(exp1,k1,exp2,k2,k2b,k3):
    ...
    legs=[f".{r1}{ymd1}C{int(k1)}x-1", f".{r2}{ymd2}C{int(k2)}x1", f".{r2}{ymd2}C{int(k2b)}x1", f".{r1}{ymd1}C{int(k3)}x-1"]
...
def batman_value_from_df(one_min_df: pd.DataFrame, exp1: str, k1: float, exp2: str, k2: float, k2b: float, k3: float, root1=None, root2=None):
    """
    Revaloriza estructura Batman 4-legs: -1C@k1(exp1) +1C@k2(exp2) +1C@k2b(exp2) -1C@k3(exp1)

    Args:
        one_min_df: DataFrame con snapshot del mercado
        exp1: Expiración de k1 y k3 (front, short legs)
        k1: Strike de primera call corta
        exp2: Expiración de k2 y k2b (back, long legs)
        k2: Strike de primera call larga (+1C)
        k2b: Strike de segunda call larga (+1C)
        k3: Strike de segunda call corta
        root1: Root esperado para exp1 (SPX/SPXW)
        root2: Root esperado para exp2 (SPX/SPXW)

    Returns:
        dict con net_credit (puntos SPX), leg1/leg2/leg2b/leg3 (bid/ask/mid/root/strike)
    ...
    try:
        ...
        # Buscar strikes k2 y k2b en exp2
        r2 = sub2_filt.loc[np.isclose(sub2_filt["strike"], float(k2), atol=1e-6)]
        r2b = sub2_filt.loc[np.isclose(sub2_filt["strike"], float(k2b), atol=1e-6)]

        if r1.empty or r2.empty or r2b.empty or r3.empty:
            return None
        ...
        row2b, tie2b = select_best_strike_row(r2b, base_mid=None)
        row3, tie3 = select_best_strike_row(r3, base_mid=None)

        if row1 is None or row2 is None or row2b is None or row3 is None:
            return None

        mid1 = get_mid_from_row(row1)
        mid2 = get_mid_from_row(row2)
        mid2b = get_mid_from_row(row2b)
        mid3 = get_mid_from_row(row3)

        if not np.isfinite(mid1) or not np.isfinite(mid2) or not np.isfinite(mid2b) or not np.isfinite(mid3):
            return None
        ...
        leg2b = _build_leg(row2b, r2b.index[0] if len(r2b) > 0 else None, mid2b, tie2b, root2_pref_applied, root2_chosen)
        leg3 = _build_leg(row3, r3.index[0] if len(r3) > 0 else None, mid3, tie3, root1_pref_applied, root1_chosen)

        # net_credit = -mid1 + mid2 + mid2b - mid3 (puntos SPX)
        # CREDIT negativo = cobramos (favorable)
        net_credit = float(-mid1 + mid2 + mid2b - mid3)
...
# ================== CÁLCULO ESTRATEGIA ==================
def compute_strategy_metrics(spot,r_base,exp1,dte1,k1,exp2,dte2,k2,k2b,k3,precache):
    ...
    d2,t2,p2,iv2,bid2,ask2,last2 = greeks_price(calls2_idx,exp2,k2,T2,r2,q2)    # +1C
    d2b,t2b,p2b,iv2b,bid2b,ask2b,last2b = greeks_price(calls2_idx,exp2,k2b,T2,r2,q2)  # +1C
    ...
    delta_total = -1*dz(d1) + 1*dz(d2) + 1*dz(d2b) - 1*dz(d3)
    theta_total = -1*dz(t1) + 1*dz(t2) + 1*dz(t2b) - 1*dz(t3)

    p1z, p2z, p2bz, p3z = dz(p1), dz(p2), dz(p2b), dz(p3)
    net_credit = (-1*p1z) + (1*p2z) + (1*p2bz) - (1*p3z)  # puntos SPX
    ...
    pnl_long2  = pnl_leg_at_S_T(+1, k2, p2z, S_PNL, T_rem_long, r2, iv2)
    pnl_long2b = pnl_leg_at_S_T(+1, k2b, p2bz, S_PNL, T_rem_long, r2, iv2b)
    pnl_short3 = pnl_leg_at_S_T(-1, k3, p3z, S_PNL, 0.0,        r1, iv3)
    pnl_total  = pnl_short1 + pnl_long2 + pnl_long2b + pnl_short3
    ...
    if tau > 0 and (iv2 is not None) and not (isinstance(iv2,float) and math.isnan(iv2)):
        ...
        if (S0 >= k_lo) and (S0 < k_hi):
            ...
            val_long2  = 1.0 * bs_call_price_safe(S0, float(k2), tau, r2, sigma2)
            val_long2b = 1.0 * bs_call_price_safe(S0, float(k2b), tau, r2, sigma2)
            value_t1   = val_short1 + val_short3 + val_long2 + val_long2b
            pnl_dv_points = value_t1 - net_credit
            death_valley  = S0
        else:
            value_lim = (float(k1) + float(k3)) - 1.0*float(k2)*math.exp(-r2*tau) - 1.0*float(k2b)*math.exp(-r2*tau)
    ...
    iv2b_clean = None if (iv2b is None or (isinstance(iv2b,float) and math.isnan(iv2b))) else float(iv2b)
    EarL_pts = None; EarR_pts = None
    if tau > 0 and iv2_clean is not None and iv2b_clean is not None:
        EarL_pts = 1.0*bs_call_price_safe(float(k1), float(k2), tau, r2, iv2_clean) + 1.0*bs_call_price_safe(float(k1), float(k2b), tau, r2, iv2b_clean) - net_credit
        EarR_pts = (-(float(k3) - float(k1))) + 1.0*bs_call_price_safe(float(k3), float(k2), tau, r2, iv2_clean) + 1.0*bs_call_price_safe(float(k3), float(k2b), tau, r2, iv2b_clean) - net_credit

    # UEL Infinita: pérdida máxima plana de la oreja derecha al llegar T1
    # Las dos cortas T1 vencen, las largas (+1C@k2 +1C@k2b) T2 siguen vivas
    # Fórmula correcta: valor_limite - net_credit, donde valor_limite = (K1 + K3) - PVK2 - PVK2b
    # PVK2 = K2·exp(-r·ΔT) es el valor presente del strike K2 descontado de T2 a T1
    UEL_inf_USD = None
    if tau > 0:
        # Calcular los strikes descontados de las largas (K2 y K2b)
        PVK2 = float(k2) * math.exp(-r2 * tau)
        PVK2b = float(k2b) * math.exp(-r2 * tau)
        ...
        UEL_inf_pts = (float(k1) + float(k3)) - 1.0*PVK2 - 1.0*PVK2b - net_credit
        # Convertir a USD (multiplicador 100 para SPX)
        UEL_inf_USD = round(100.0 * UEL_inf_pts, 2)
    elif tau <= 0:
        # Si no hay diferencia temporal, usar K2/K2b sin descuento
        PVK2 = float(k2)
        PVK2b = float(k2b)
        UEL_inf_pts = (float(k1) + float(k3)) - 1.0*PVK2 - 1.0*PVK2b - net_credit
        UEL_inf_USD = round(100.0 * UEL_inf_pts, 2)
    ...
    iv2b_fmt = None if (iv2b is None or (isinstance(iv2b,float) and math.isnan(iv2b))) else round(float(iv2b),6)
    ...
    row_id = f"{exp1}/{exp2}|{k1}/{k2}/{k2b}/{k3}"
    ...
    # ATM_back es el promedio de IVs del body (back expiration: k2 y k2b)
    atm_back = None
    valid_body = []
    if iv2 is not None and not (isinstance(iv2, float) and math.isnan(iv2)):
        valid_body.append(float(iv2))
    if iv2b is not None and not (isinstance(iv2b, float) and math.isnan(iv2b)):
        valid_body.append(float(iv2b))
    if valid_body:
        atm_back = sum(valid_body) / len(valid_body)
    ...
    out = {
        ...
        "k1": int(k1), "k2": int(k2), "k2b": int(k2b), "k3": int(k3),
        ...
        "delta_k2b": None if math.isnan(d2b) else round(d2b,6),
        "theta_k2b": None if math.isnan(t2b) else round(t2b,6),
        ...
        "price_bid_long2b": None if math.isnan(bid2b) else round(bid2b,4),
        "price_ask_long2b": None if math.isnan(ask2b) else round(ask2b,4),
        "price_last_long2b": None if math.isnan(last2b) else round(last2b,4),
        "price_mid_long2b":  None if math.isnan(p2b)   else round(p2b,4),
        ...
        "pnl8000_long2b": round(pnl_long2b,2),
    ...
    for fr, suf in zip(forward_fracs, frac_suffixes):
        ...
        try:
            ...
            for ts_hhmm in intraday_timestamps:
                ...
                # Revalorizar batman 4-legs en este timestamp
                ...
                if first_valid_timestamp is None:
                    ...
                    for leg_label, leg_info in (("k1", bv_details.get("leg1")),
                        ...
                        if leg_label == "k1":
                            ...
                            expected_mid = row_dict.get("price_mid_long2b")
                        else:  # k3
...
def _process_k1_candidate(args):
    ...
    Retorna lista de candidatos Batman 4-legs generados para este k1.
    """
    (k1, s2_list, s2b_list, s3_list, e1, e2, dte_map, spot, r_base, date_es_str, hhmm_es, _hhmm_us,
    ...
    for k2 in s2_list:
        ...
        # Loop k2b (segunda +1C de DTE2)
        for k2b in s2b_list:
            if USE_LIQUIDITY_FILTERS and not leg_liquidity_ok_idx(calls2_idx, k2b, MIN_OI_BACK, MIN_VOL_BACK):
                continue
            mid2b = quick_mid_idx(calls2_idx, k2b)
            if math.isnan(mid2b):
                continue

            # Loop k3 (segunda -1C de DTE1)
            for k3 in s3_list:
                ...
                # Prefiltro de crédito (4 legs: -1C +1C +1C -1C)
                pre_net_credit = (-mid1) + mid2 + mid2b - mid3
                if not (PREFILTER_CREDIT_MIN <= pre_net_credit <= PREFILTER_CREDIT_MAX):
                    continue

                # Validar orden de strikes (k1 < k2 < k2b < k3)
                if not (k1 < k2 < k2b < k3):
                    continue

                # Generar URL (4 legs)
                url = make_url(e1, k1, e2, k2, k2b, k3)
                ...
                # Computar métricas (4 legs)
                met = compute_strategy_metrics(
                    spot, r_base, e1, dte_map[e1], k1, e2, dte_map[e2], k2, k2b, k3, precache_local
                )
...
def main():
            ...
            for t_idx, target_hhmm_us in enumerate(TARGET_HHMMS_US if not IGNORE_TARGET_MINUTE else ["ALL"], start=1):
                ...
                for e1 in exp_A:
                    ...
                    for e2 in exp_B:
                        ...
                        # Generar candidatos K2 basados en delta (primera +1C de DTE2)
                        ...
                        # Generar candidatos K2b basados en delta (segunda +1C de DTE2)
                        s2b_list = gen_k2b_candidates_by_delta(
                            calls2_idx, strikes2, spot, dte_map[e2], r_base,
                            delta_min=K2b_DELTA_MIN,
                            delta_max=K2b_DELTA_MAX
                        )

                        if not s2_list or not s2b_list:
                            continue

                        # Crear tareas para cada k1 (4 legs: -1C +1C +1C -1C)
                        for k1 in s1_list:
                            task = (
                                k1, s2_list, s2b_list, s3_list, e1, e2, dte_map, spot, r_base,
                ...
                # ============================================================
                # EJECUTAR PROCESAMIENTO - Modo Incremental o Batch
                # ============================================================
                if tasks:
                    import os as os_module

                    # Preparar archivo Parquet de salida
                    parquet_path = temp_dir / f"day_{pick_idx:03d}_{date_str_us.replace('-', '')}.parquet"

                    if INCREMENTAL_MODE:
                        # ========================================================
                        # MODO INCREMENTAL: Procesar y escribir inmediatamente
                        # ========================================================
                        print(f"     [INCREMENTAL] Procesando {len(tasks)} tareas k1 con modo incremental...")

                        # Crear archivo temporal CSV para escritura incremental
                        temp_csv_fd, temp_csv_path = tempfile.mkstemp(suffix=".csv", prefix="batman_incr_", dir=str(temp_dir))
                        ...
                        for task_idx, task in enumerate(tasks):
                            if INCREMENTAL_WORKERS == 1:
                                # Procesamiento secuencial (sin overhead de multiprocessing)
                                result_rows = _process_k1_candidate(task)
                            else:
                                # Micro-batch con workers limitados
                                with ProcessPoolExecutor(max_workers=INCREMENTAL_WORKERS) as micro_executor:
                                    result_rows = list(micro_executor.map(_process_k1_candidate, [task]))[0]
                            ...
                            # Progress cada 10 tareas
                            if (task_idx + 1) % 10 == 0:
                                print(f"       Procesadas {task_idx + 1}/{len(tasks)} tareas ({total_candidates_written} candidatos)")

                        # Cerrar archivo CSV
                        if csv_file_handle:
                            csv_file_handle.close()

                        print(f"     [INCREMENTAL] Completado. {total_candidates_written} candidatos generados.")

                        # Convertir CSV a Parquet y continuar con el flujo normal
                        if total_candidates_written > 0:
                            print(f"     [CONVERT] Convirtiendo CSV temporal a Parquet...", end='', flush=True)
                            df_day = pd.read_csv(temp_csv_path, low_memory=False)
                            ...
                            print(f" ✓ ({len(df_day)} filas, {mem_mb:.1f} MB)")
                        ...
                        # ========================================================
                        # MODO BATCH ORIGINAL: Procesar todo en paralelo
                        # ========================================================
                    ...
                    print(f"     [INFO] Sin tareas para procesar en este timestamp.")

            # Reset rows para siguiente iteración (solo en modo batch)
    ...
    # ============================================================
    # ESCRITURA INCREMENTAL DEL CSV FINAL
    # ============================================================
    # Si el DataFrame es grande o se usó external sort, escribir por chunks
    # para evitar picos de memoria en la conversión a CSV
    df_size_mb = df.memory_usage(deep=True).sum() / (1024**2) if hasattr(df, 'memory_usage') else 0
    use_incremental_write = (df_size_mb > 500) or (sort_strategy in ["external_sort", "external_sort_minimal"])

    if use_incremental_write and checkpoint_path.exists():
        print(f"\n[ESCRITURA INCREMENTAL] DataFrame grande ({df_size_mb:.1f} MB) - escribiendo CSV por chunks...")

        # Leer desde el checkpoint parquet y escribir por chunks
        pq_file = pq.ParquetFile(checkpoint_path)
        first_chunk = True
        ...
        for batch in pq_file.iter_batches(batch_size=50_000):
            ...
            # Aplicar conversiones numéricas al chunk
            for c in num_cols:
                if c in df_chunk.columns:
                    df_chunk[c] = pd.to_numeric(df_chunk[c], errors="coerce")

            # Escribir con header solo la primera vez
            df_chunk.to_csv(batch_out_path, mode='a' if not first_chunk else 'w',
                           index=False, header=first_chunk, encoding='utf-8-sig', na_rep="")
            ...
            if total_rows_written % 100_000 == 0:
                print(f"  Escritas {total_rows_written:,} filas...")

        print(f"\n[✓] CSV guardado incrementalmente en: {batch_out_path} | Filas: {total_rows_written:,}")
    else:
        # DataFrame pequeño: escritura normal en memoria
    ...
    # ============================================================
    # OPTIMIZACIÓN: Evitar copia completa del DataFrame si es muy grande
    # ============================================================
    # Si se usó external sort o el DataFrame es grande, trabajar con referencias
    # y hacer solo las modificaciones necesarias para minimizar uso de memoria
    df_is_large = (df_size_mb > 500) or (sort_strategy in ["external_sort", "external_sort_minimal"])

    if df_is_large and checkpoint_path.exists():
        print(f"[OPTIMIZACIÓN] DataFrame grande detectado - procesando sin copia completa...")
        # Leer solo las columnas necesarias del checkpoint
        required_cols_for_copy = ['dia', 'PnL_fwd_pct_25_mediana', 'PnL_fwd_pct_25', 'exp1', 'exp2', 'k1', 'k2', 'k2b', 'k3', 'net_credit']
        df_copy = pd.read_parquet(checkpoint_path, columns=[c for c in required_cols_for_copy if c in pd.read_parquet(checkpoint_path, nrows=0).columns])
        print(f"  Cargadas solo {len(df_copy.columns)} columnas necesarias (ahorro de memoria)")
    else:
        # DataFrame pequeño: copia normal
        df_copy = df.copy()
    ...
    if col_filter not in df_copy.columns:
        ...
        if len(df_copy) > 0:
            ...
            # ============================================================
            # GUARDAR CSV COPIA (incremental si es grande)
            # ============================================================
            df_copy_size_mb = df_copy.memory_usage(deep=True).sum() / (1024**2)

            if df_copy_size_mb > 500:
                print(f"\n[CSV COPIA] DataFrame grande ({df_copy_size_mb:.1f} MB) - escribiendo incrementalmente...")

                # Escribir por chunks
                chunk_size = 50_000
                n_chunks = (len(df_copy) + chunk_size - 1) // chunk_size
                ...
                for i in range(n_chunks):
                    ...
                    # Primera escritura incluye header
                    chunk.to_csv(batch_out_path_copy, mode='a' if i > 0 else 'w',
                               index=False, header=(i == 0), encoding='utf-8-sig', na_rep="")

                    del chunk
                    gc.collect()

                    if (i + 1) % 5 == 0:
                        print(f"  Escritos {end_idx:,}/{len(df_copy):,} filas...")

                print(f"[CSV COPIA] ✓ Guardado incrementalmente en: {batch_out_path_copy}")
            else:
                # DataFrame pequeño: escritura normal
